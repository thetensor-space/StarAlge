...
import "prelims.m":  SolveSystem, __RF_SETUP, IdentifyBasis, __SymAlt;
...
__default_adj := function (Forms)
     ...
     /* Added the multilinear tech to solve for sym and alt quicker. (5/23/16 JM) */
     /* basis for SYM and ALT */     
     SYM, ALT := __SymAlt(Forms);
     SYM_BASIS := [ MA!Eltseq (b) : b in SYM ];
     ALT_BASIS := [ MA!Eltseq (b) : b in ALT ];
...
__default_adjOLD := function (Forms)
...
AdjointsOfAForm := function (F, Auto) 
     ...
     SAI_data`transitionMatrix := Identity (STAN);
...
// added by PAB on 5/13/16
SAI_data`normaliserOrder := STAN`StarAlgebraInfo`normaliserOrder;
SAI_data`normaliserGenerators := 
        [ STAN`StarAlgebraInfo`normaliserGenerators[i] @ STANtoADJ :
                i in [1..#STAN`StarAlgebraInfo`normaliserGenerators] ];
SAI_data`primitiveIdempotents := STAN`StarAlgebraInfo`primitiveIdempotents;
...
intrinsic AdjointAlgebra (S::SeqEnum : 
...
//"dim sol =", Dimension (sol);
     ...
     basW := [ MSe![ Vector (sol.i)[j] : j in [n^2+1..n^2+e^2] ] : 
                                     i in [1..Ngens (sol)] ];
     DerW := sub < MSe | [ MSe!basW[i] : i in [1..#basW] ] >;
