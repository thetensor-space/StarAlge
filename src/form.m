...
__transform_form := function (form, d, q, type)
    ...
    C := TransformForm (M, type : Restore := true);
...
BilinearForm := function (G)
     ...
     N := GModule (sub < Generic (G) | 
                    [ Transpose (G.i^-1) : i in [1..Ngens (G)] ] >);                    
     X := AHom (M, N);
...
StandardReflexiveForm := function (name, d, K)                                       
     assert d gt 0;
     if d eq 1 then
...
__standard_isometry_group := function (name, d, k)
	case name :
        ...
        when "orthogonalcircle" :
        	if d lt 3 then // so d=1
        		G := sub<GL(d,k) | GL(d,k)![-1]>;
        	else
         		G := GO (d, k);
         	end if;
...
intrinsic PGroupToForms (P::GrpPC, p::RngIntElt) -> SeqEnum
   ...
   { The system of forms determined up to 
     ...
     require #gamma[#gamma] eq 1 : "argument 1 is not nilpotent.";
...
intrinsic PGroupToForms (P::GrpMat, p::RngIntElt) -> SeqEnum
   ...
   { The system of forms determined up to isometry by the 
     ...
     require IsElementaryAbelian (DP) : 
                 "aruument 1 is not a class 2 p-group";
