...
__tensor_perm := function (g, d, posns, r)
     k := BaseRing (Parent (g));
     G := Identity (MatrixAlgebra (k, d));
     m := Degree (Parent (g));
     assert m eq #posns;
     Z := MatrixAlgebra (k, r)!0;
     I := Identity (MatrixAlgebra (k, r));
     for i in [1..m] do
         for j in [1..m] do
            if g[i][j] eq 0 then
               InsertBlock (~G, Z, posns[i], posns[j]);
            else   // g[i][j] = 1
               InsertBlock (~G, I, posns[i], posns[j]);
            end if;
         end for;
     end for;
return G;
...
intrinsic RecogniseStarAlgebra (A::AlgMat) -> BoolElt
   ...
   { Constructively recognise the *-algebra A. }
     ...
     for i in [1..#data`srComponents] do
         ...
         if btype eq "classical" then // happens here
         ...
         /* embed normaliser generators in <A> */
         norm_order *:= S`StarAlgebraInfo`normaliserOrder;
         for s in S`StarAlgebraInfo`normaliserGenerators do
            x := InsertBlock (u, s, pos, pos);
            xx := COBi * x * COB;
            Append (~norm_gens, xx);   
         end for;
           
         /* embed primitive self-adjoint idempotents in <A> */
         z := A!0;
         for y in S`StarAlgebraInfo`primitiveIdempotents do
             x := InsertBlock (z, y, pos, pos);
             xx := COBi * x * COB;
// commented out 5/6/2011: we only want the sum of the idempotents to be
// self-adjoint in the symplectic case.
//             assert ((xx^2 eq xx) and (xx @ A`Star eq xx));
             Append (~idempotents, xx);
         end for;
     ...
     norm_order *:= Characteristic (BaseRing (A))^(#Jminus);
//"(3) norm_order =", norm_order;
     if #Jminus gt 0 then
	Op_gens := [ gl!(__power_series_trick (Generic (A)!z)) : z in Jminus ];
        sharp_gens cat:= Op_gens;
        norm_gens cat:= Op_gens; 
     end if;

     assert forall{x : x in sharp_gens | IsUnitaryElement (A, x)};
       
     /* form the permutation partition */
     m := #data`srComponents;
     comp_dims := [ Degree (data`srComponents[i]) : i in [1..m] ];
     assert Degree (A) eq &+ comp_dims;
     types := [ ];
     for i in [1..m] do
         par := data`srComponents[i]`StarSimpleInfo`simpleParameters;
         type := < comp_dims[i] , par[1] , par[2] , par[3] >;
         Append (~types, type);
     end for;
     dist := Set (types);
     partition := [ [ i : i in [1..m] | types[i] eq t ] : t in dist ];

     /* construct the permutation generators */
     perm_gens := [ ];
     perm_order := 1;
     d := Degree (A);
     k := BaseRing (A);
	 ...
	 l := #part;
         if (l gt 1) then
//	    "adding a perm generator";
	    r := types[part[1]][1];
            positions := [ ];
            for i in [1..l] do
               if part[i] gt 1 then
	          pos := 1 + &+[ comp_dims[j] : j in [1..part[i]-1] ];
               else
                  pos := 1;
               end if;
	    ...
	    G := SymmetricGroup (l);
            for g in Generators (G) do
               permg := PermutationMatrix (k, g);
               permG := __tensor_perm (permg, d, positions, r);
               Append (~perm_gens, COBi * permG * COB);
	    end for;
            perm_order *:= #G;
	 end if;
     end for;

/* test to see which of the normaliser generators actually work */
/*
"number of norm gens:", #norm_gens;
"number of good norm gens:",
  #[ x : x in norm_gens | 
     forall { a : a in Generators (A) | 
      (x * a * x^-1 in A) and 
      (x * (a @ A`Star) * x^-1 eq (x * a * x^-1) @ A`Star) } ];

"number of perm gens:", #perm_gens;
"number of good perm gens:",
  #[ x : x in perm_gens | 
     forall { a : a in Generators (A) | 
      (x * a * x^-1 in A) and 
      (x * (a @ A`Star) * x^-1 eq (x * a * x^-1) @ A`Star) } ];
*/
     ...
     data`normaliserGenerators := norm_gens cat perm_gens;
     data`normaliserOrder := norm_order * perm_order;
//"(4) norm_order =", data`normaliserOrder;
//"GL order =", #GL (d, k);
/* this issue will go away once the stabiliser part if plugged in */
if data`normaliserOrder gt #GL (d, k) then
data`normaliserOrder := #GL (d, k);
end if;
     data`primitiveIdempotents := idempotents;
